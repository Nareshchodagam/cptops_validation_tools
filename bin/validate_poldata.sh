...
check_status() {
   ...
   for i in $( seq 3 )
   do
      # If we don't see this pattern, most likely illumio will fail to start.
...
validate_db() {
   ...
   do
      ...
      case $? in
         ...
         * )
            echo $?
            echo -e "${LGREEN}######### ${HOSTNAME}: DB validation has been completed! #########${NC}"
            exit 0;;
      esac
   done
   exit 0
