...
MDH_SRC_1_DISK=$(cat /etc/flux/.${HOME}_src_1_disk_plain)
MDH_SRC_2_DISK=$(cat /etc/flux/.${HOME}_src_2_disk_plain)
OLD_MDH=$(lsblk -i -o NAME,MOUNTPOINT | grep $MDH_SRC_1_DISK -A 1 | tail -n 1 | perl -nle 'm/(md[0-9]+)/; print $1')
...
mdadm --add /dev/$MDH /dev/$MDH_SRC_1_DISK
mdadm --add /dev/$MDH /dev/$MDH_SRC_2_DISK
