...
if [ $RETVAL_SQL -ne 0 ]; then $SYSTEMCTL start postgresql && RETVAL_SQL=0 && echo "POSTGRESQL:[STARTED]"; else echo "Unable to start POSTGRESQL service" && exit 1; fi
# IF STATUS_POSTGRESQL is NOT RUNNING then START POSTGRESQL service
}

function start_ksm () {
 $SYSTEMCTL is-active --quiet ksm && RETVAL_KSM=0 && echo "KSM:[ALREADY RUNNING]" || RETVAL_KSM=1
if [ $RETVAL_SQL == 0 ] && [ $RETVAL_KSM != 0 ]; then $SYSTEMCTL start ksm && RETVAL_KSM=0 && echo "KSM:[STARTED]"; else echo "Unable to start KSM service" && exit 1; fi
# IF STATUS_POSTGRESQL is RUNNING & STATUS_KSM is NOT RUNNING then START KSM service
}

function start_val () {
$SYSTEMCTL is-active --quiet val && RETVAL_VAL=0 && echo "VAL:[ALREADY RUNNING]" || RETVAL_VAL=1
if [ $RETVAL_SQL == 0 ] && [ $RETVAL_KSM == 0 ] && [ $RETVAL_VAL != 0 ]; then $SYSTEMCTL start val && echo "VAL:[STARTED]"; else echo "Unable to start VAL service" && exit 1; fi
# IF STATUS_POSTGRESQL is RUNNING & STATUS_VAL is NOT RUNNING then START VAL service
}

## Stop Services
function authval_stop () {
 echo "Please ensure that the AUTHRAD services are down on the same stack." && sleep 60
